...
* ``legal_search`` – perform a legal database search via the Indian
  Kanoon API.  Returns a list of results with titles, citations and
  metadata.  Requires an API token set in the ``INDIAN_KANOON_TOKEN``
  environment variable.  If no token is available or the API call
  fails, the function returns an empty list.
...
def legal_search(query: str, *, page: int = 0, max_results: int = 10) -> List[Dict[str, Any]]:
    ...
    page: int, optional
        Which page of results to fetch.  Indian Kanoon returns 10
        results per page; page numbering starts at 0.
    ...
    -------
    List[Dict[str, Any]]
        A list of result objects.  Each object has keys:
        ``title``, ``court``, ``date``, ``citation``, ``doc_id``,
        ``link`` and ``snippet``.  If the API call fails or yields no
        results, the list will be empty.

    Notes
    -----
    This implementation uses the unofficial Indian Kanoon API
    (``https://api.indiankanoon.org``).  You must set the
    ``INDIAN_KANOON_TOKEN`` environment variable with a valid API
    token.  See the documentation at
    https://api.indiankanoon.org/ for details on obtaining a token
    and on the API parameters【443980537531437†L23-L35】.
    """
    token = os.environ.get("INDIAN_KANOON_TOKEN")
    if not token:
        # Without a token we cannot perform the search; return empty result.
        return []
    # Build the request URL.  We request JSON format if available.  If
    # the API does not support JSON, we will need to parse HTML; in
    # this case we return an empty list rather than hallucinate.
    url = (
        f"https://api.indiankanoon.org/search/?formInput={requests.utils.quote(query)}"
        f"&pagenum={page}&format=json&max={max_results}"
    )
    ...
    # The JSON structure returned by the API typically contains a
    # ``results`` list where each entry has keys like ``docid``,
    # ``title``, ``court``, ``decision_date``, ``citation``,
    # ``snippet``.  We normalise these keys.  If the structure is
    # different or empty, we return an empty list.
